%% attenzione ! %%
%% Eseguire prima il file script.m per avere in cache la variabili che questo script utilizza per i modelli

%% forecast regressione lineare
x = [tNordEst.NE_DIABETE(1:end-5,:) tNordEst.NE_MA_ALLERGICHE(1:end-5,:) tNordEst.NE_ECCESSO_PESO(1:end-5,:)];
...
estModel = estimate(modelNE, y, params)
...
% forecast regressione multipla (Previsione un passo in avanti)
...
params = [1 1 1 1];
x_regDin = [tNordEst.NE_DIABETE(1:end-5,:) tNordEst.NE_MA_ALLERGICHE(1:end-5,:) tNordEst.NE_ECCESSO_PESO(1:end-5,:)];
y = tNordEst.NE_IPERTENSIONE(1:end-5);

funzioneMap = @(params) map(params, x_regDin, NE_lm1.Coefficients.Estimate(1), NE_lm1.Coefficients.Estimate(2), NE_lm1.Coefficients.Estimate(3), NE_lm1.Coefficients.Estimate(4));
modelNE = ssm(funzioneMap)

[estModel,estParams] = estimate(modelNE, y, params);
x_regDin = [ones(length(tNordEst.NE_DIABETE(1:end-5,:)),1) tNordEst.NE_DIABETE(1:end-5,:) tNordEst.NE_MA_ALLERGICHE(1:end-5,:) tNordEst.NE_ECCESSO_PESO(1:end-5,:)];
x_last5 = [ones(length(tNordEst.NE_DIABETE(end-4:end,:)),1) tNordEst.NE_DIABETE(end-4:end,:) tNordEst.NE_MA_ALLERGICHE(end-4:end,:) tNordEst.NE_ECCESSO_PESO(end-4:end,:)];
...
x = [tNordEst.NE_DIABETE(1:end-5,:) tNordEst.NE_MA_ALLERGICHE(1:end-5,:)];
y = tNordEst.NE_IPERTENSIONE(1:end-5,:);
x_last5 = [tNordEst.NE_DIABETE(end-4:end,:) tNordEst.NE_MA_ALLERGICHE(end-4:end,:)];
...
subplot(2,2,2)
    [S,AX,BigAx,H,HAx] = plotmatrix([tNordEst.NE_DIABETE(1:end-5,:) tNordEst.NE_MA_ALLERGICHE(1:end-5,:)], res)
...
hold on
    %            reg lineare, reg dinamica filter, reg dinamica smooth, arma
    y_stimate = [ypred(end-4:end,:) , y3_flt(end-4:end,:) , y3_smo(end-4:end,:) , yF , y(end-4:end,:)] 
    plot(y_stimate)
    legend('Regressione lineare', 'regressione dinamica filter', 'regressione dinamica smoother', 'ARMA', 'Y Vera')
hold off