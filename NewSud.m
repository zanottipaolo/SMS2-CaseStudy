...
% forecast regressione dinamica (Previsione un passo in avanti)
...
funzioneMap = @(params) map(params, x1, SU_lm1.Coefficients.Estimate(1), SU_lm1.Coefficients.Estimate(2), SU_lm1.Coefficients.Estimate(3), SU_lm1.Coefficients.Estimate(4));
modelSU = ssm(funzioneMap)
...
for k = 1:m
    ...
    for i=2:20
        ...
        y_sim(i,k) = estimate_model.Intercept + estimate_model.Beta(1)*x(i,1) + estimate_model.Beta(2)*x(i,2) + cell2mat(estimate_model.MA)*err(i-1) + err(i);
    end
end
