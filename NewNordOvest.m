...
funzioneMap = @(params) map(params, x1, NO_lm1.Coefficients.Estimate(1), NO_lm1.Coefficients.Estimate(2), NO_lm1.Coefficients.Estimate(3), NO_lm1.Coefficients.Estimate(4));
modelNO = ssm(funzioneMap)
...
% ARMA(1,0,0) modello con migliore rapporto BIC e MSE, con coefficienti
...
for k=1:m
    ...
    for i=2:20
        ...
        y_sim(i,k) = estimate_model.Intercept + estimate_model.Beta(1)*x(i,1) + estimate_model.Beta(2)*x(i,2) + cell2mat(estimate_model.AR)*err(i-1) + err(i);
    end
end
